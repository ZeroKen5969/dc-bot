...
export = {
    name: 'foreverbanmember',
    aliases: ["fbm"],
    description: '封鎖團隊成員並加入黑名單',
                ...
                async end(collected) {
                    ...
                    for (let i = 0; i < allIndex.length; ++i) {
                        ...
                        const results = await Promise.all([
                            teamMgr.banMember(crewid, gameId),
                            blackListDao.addBlackId(gameId)
                        ]);

                        if (results[0] == null) {
                            msgBuf.push(`\`${crewname}\` | 玩家\`${may[index - 1].data.Name} (${gameId})\`不在團隊名單!`);
                        } else {
                            msgBuf.push(`\`${crewname}\` | 玩家\`${may[index - 1].data.Name} (${gameId})\`已被剔除 & 封鎖 & 黑單!`);
                        }
                    }
